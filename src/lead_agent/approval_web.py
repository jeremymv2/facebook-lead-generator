...
class LocalApprovalController:
    ...
    def __init__(self, service: LocalApprovalService, *, csrf_token: str | None = None) -> None:
...
<head>
  ...
  <style>
    ...
    main {{ width: min(860px, calc(100% - 28px)); margin: 28px auto 60px; }}
    header {{ margin-bottom: 20px; }}
    h1 {{ margin-bottom: 4px; }}
    .safety {{ color: #57606a; margin-top: 0; }}
    .card, .empty {{ background: white; border: 1px solid #d8dee4; border-radius: 12px;
...
def run_local_approval_dashboard(
    ...
    *,
    ...
    controller = LocalApprovalController(service)
